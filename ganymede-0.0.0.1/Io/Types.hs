...
data EnvFilter = EFail | EFLimit [IoID]
...
-- (* Interpreter *)
type IoAssoc = (IoID, IORef IoCont)

type IoVAssoc = (IoID, IORef (IORef IoCont))

type IoAssocs = M.HashMap IoID (IORef IoCont)

type IoVAssocs = M.HashMap IoID (IORef (IORef IoCont))
