...
import Io.Env (bind, vBind, build_local_conts, link_external_conts, standard_environment)
import Io.Parser (parseFromFile, parse_io_module)
import Io.Types
import Io.Util (find_file, ref, splitString, subStr)

import Prelude hiding (catch, lookup)
import Control.Exception (catch, catches, onException, throw, Handler(..))
...
loadAST file_path = do {
  Right (imps, exps, dcls, expr) <- parseFromFile parse_io_module file_path;  -- Parser
  return ({- fetch default_imports ++ -}imps, exps, dcls, expr)
  }
...
load_interfaces interfaces imported_name importedAST@(imps, _, _, _) =
  let split_n_foldLM :: (Monad m) => (acc -> x -> m (acc, y)) -> acc -> [x] -> m (acc, [y])
      ...
      load_interfaces_ importing_name interfaces (imported_id, imps_by_imp) = do {  
        name <- find_module_name importing_name (idToFilename imported_id);
        fmap (\x -> (x, (name, imps_by_imp))) $
          case L.lookup name interfaces of
            Nothing -> loadAST name >>= (load_interfaces interfaces name)
            _       -> return interfaces }
  ...
  in do
    ...
    ; return . M.toList . M.insert imported_name (exps, imps', importedAST) $
             foldr (uncurry M.insert) M.empty is'
    }
 
{- Builds an environment for a module -}
build_env :: [IoModuleInterface] -> IoModuleName -> IoEnv
build_env interfaces name = foldl' Io.Env.vBind en'
    (map (\ (_, vi, ex) -> (vi,ref (CLocal ex))) vds) where
  Just (_,imps,(_, _, (ds, vds),ex)) = L.lookup name interfaces
  en  = foldl' imp_assocs Io.Env.standard_environment imps
  en' = foldr (flip Io.Env.bind .  (\ (_, i, ex) -> (i, CLocal ex))) en ds
  imp_assocs en (imp_name, is) = foldr (\ i -> flip Io.Env.bind (i, CExternal imp_name)) en imp_ids where
...
build_modules traceP interfaces = do
  { mapM_ (Io.Env.build_local_conts traceP >> Io.Env.link_external_conts enmp) ens
...
catch_module_errors f a =
  catches (f a) $
    [ Handler (\(IoParserError pos) ->
      throw (IoParserError pos))
    , Handler (\(IoLexerError pos) ->
      throw (IoLexerError pos))
    , Handler (\(IoModuleNotFound importer imported) ->
      throw (IoModuleNotFound importer imported))
    , Handler (\(IoUndefinedExportError id mname) ->
      throw (IoUndefinedExportError id mname))
    , Handler (\(IoUndefinedImportError id mname) ->
      throw (IoUndefinedImportError id mname))
    ]
