...
import Control.Exception (Handler(..), catch, catches, onException, throw)
import Control.Monad (when)
import Data.HashMap.Lazy as M (empty, filterWithKey, insert, lookup, toList)
import Data.List (foldl', intercalate)
import Debug.Trace (trace)

{- Debug status vars -}
appl_pos = ref freshPos
lambda_pos = ref freshPos
...
interpret (CExpr (EAppl p i es flt) en _, []) trace_flag =
  let new_en = filter_en flt trace_flag en in do {
    appl_pos <-- p;
    (if trace_flag
      then trace (Messages.application_trace (fetch appl_pos) i)
      else id)
...
interpret (CExpr (ELambda p is e flt) en _, ps) trace_flag =
  ...
  if (iLen /= pLen)
   then throw (IoMismatchError "lambda" p iLen pLen)
     ...
     foldr (.) id (if trace_flag 
       then map trace (Messages.print_lambda_trace p cmb)
       else [])
...
interpret (CExpr (EPutVar _ vi a e _) en _, _) trace_flag = 
