...
import Data.HashMap.Lazy (elems, size)
...
cont_size (CExpr _ (en, _) False) =
    (foldl' ((.(fst . cont_size . fetch)) . (+)) 1 $ elems en, size en) 
...
print_expr level e' = case e' of {
    ...
    printApplParams level ps = case ps of
          [] -> ""
          [e@(ELambda _ _ _ _)] -> " " ++ print_expr level e
          [e] -> ";" ++ print_expr level e
          (e:es) -> printParam level e ++ printApplParams level es
    printParam :: Int -> IoExpr -> String
    printParam level ps = " " ++ case ps of
          EId _ i      ->  i
          EInt _ n     -> show n
          EString _  s -> show s
          e            -> "(" ++ print_expr level e ++ ")"
