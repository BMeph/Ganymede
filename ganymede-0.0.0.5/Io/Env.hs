...
import Io.Types
import Io.Util ((<--), fetch, first, ref, second)
...
fBind  = first  . (:) . second ref
fVBind = second . (:) . second ref
-- ^ fBind  (i, c)  (en, vEn) = (insert i ref c en, vEn                  )
-- ^ fVBind (vi, c) (en, vEn) = (en                  , insert i ref c vEn)
...
build_cont decl en ex = case ex of
    ...
    EPrimitive p -> CPrim (fromJust (M.lookup p Prim.commandMap))
    _             -> CExpr ex en decl
...
build_local_conts traceP ten@(en, ven) = mapM_ build en >>
                                           mapM_ vBuild ven where
...
link_external_conts ens ten@(en, _) = mapM_ link . filter (extRef . snd) $
                                        map (second fetch) en where
    ...
    get_cont visited_ms (CExternal mname) =
      if mname `elem` visited_ms
        then throw (IoUndefinedExportError i mname)
              ...
              (envLookup (fromJust (lookup mname ens)) i)
    get_cont _ c = c

{- Returns an updated environment pair, given an ID/env list of possible substitutes.
extCUpds :: [(IoID, IoEnv)] -> IoEnv -> IoAssocs
extCUpds ens ten@(en, _) = foldr uncIns en . map link .
       filter (extRef . snd) $ map (second fetch) en where
  extRef (CExternal _) = True
  extRef         _     = False
